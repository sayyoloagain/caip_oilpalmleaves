...
public class MyProject {
    ...
    private static int nClasses = 3;        // no of class
    private static List<String> labels;     // label the image
    ...
    private static double[][] priorBoxes = {{1, 3}, {2.5, 6}, {3, 4}, {3.5, 8}, {4, 9}};    // for predict object in

    // hyperparameters for training
    private static int batchSize = 5;   // no of samples propagated through the network
    private static int nEpochs = 200;     // no of pass of all training samples
    private static double learningRate = 1e-4;  // controls model response to estimated error

    // model
    private static File modelFilename = new File(System.getProperty("user.dir"), "generated-models/PalmOilDetection_tinyyolo9.zip"); // create model after trained
    private static ComputationGraph model; // network configuration
    ...
    private static Scalar[] colormap = {RED, YELLOW, GREEN};        // colour for each class (Red = deficiency, Yellow = excess, Green = normal)
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (modelFilename.exists()) {
            ...
            System.out.println(model.summary(InputType.convolutional(Iterator.yoloHeight,Iterator.yoloWidth,nClasses)));
    ...
    private static Mat drawResults(List<DetectedObject> objects, Mat mat, int w, int h) {
        for (DetectedObject obj : objects) {
            ...
            rectangle(
                    mat,
                    new Point(x1 + 600, y2 - 2),
                    new Point(x1 + 600 + textSize.get(0), y2 - 2 - textSize.get(1)),
                    colormap[obj.getPredictedClass()],
                    FILLED,
                    0,
                    0
            );
            putText(mat, labeltext, new Point(x1 + 600, y2 - 2), FONT_HERSHEY_DUPLEX, 1, RGB(0, 0, 0));
        }
